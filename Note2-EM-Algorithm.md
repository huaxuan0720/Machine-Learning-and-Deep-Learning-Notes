...
2. 对每一次迭代循环，$j = 0, 1, 2, 3, \cdots, M$，我们有：  
   ...
   2.1 E步（E-Step）：在当前的模型参数$\theta^{(j)}$的条件下，计算联合分布的条件概率期望：  

$$
Q_i(z^{(i)}) = P(z^{(i)}|x^{(i)};\theta^{(j)})
$$

   2.2 M步（M-Step）：在计算出条件概率分布的期望的基础上，极大化似然函数，得到新的模型参数$\theta^{(j+1)}$的值:
